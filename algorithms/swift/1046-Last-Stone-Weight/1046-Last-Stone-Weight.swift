...
class Solution {
    func lastStoneWeight(_ stones: [Int]) -> Int {
		// Create a max heap.
        var maxHeap = Heap<Int>(sort: { lhs, rhs in
            return lhs > rhs
        })
        // Insert weights into max heap.
        for w in stones {
            maxHeap.insert(w)
        }
        // If max heap is not empty, remove the top 2.
        while !maxHeap.isEmpty {
            let w1 = maxHeap.remove()
            let w2 = maxHeap.remove()
            
            if let w1 = w1, let w2 = w2 {
			    // Insert difference in weight if not same.
                if w1 != w2 {
                    maxHeap.insert(w1-w2)
                }
            }
			// If last element, just return
            else if let w1 = w1, maxHeap.isEmpty {
                return w1
            }
        }
